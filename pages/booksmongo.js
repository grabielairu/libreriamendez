...
export default function Books({}) {
  ...
  const getData = async () => {
    const res = await axios.get("/api/mongodb/mongodbtest");
    const data = await res.data;
    setData(data);
  };
  ...
  const sendData = async () => {
    ...
    try {
      const resultado = await axios.post("/api/mongodb/mongotestdb", {
        title: title,
        author: author,
        genre: genre,
  ...
  const deleteBook = async (id) => {
    try {
      const resultado = await axios.delete(`/api/mongodb/mongodbtest?id=${id}`);
